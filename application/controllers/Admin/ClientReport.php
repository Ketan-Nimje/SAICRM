...
class ClientReport extends CI_Controller {
    ...
    public function GetData() {
        ...
        if($requestData['select_inq']=="A")
        {
            $status=" sc.status='A' and ";
        }
        else if($requestData['select_inq']=="D")
        {
            $status=" sc.status='D' and ";
        }
        else
        {
            $status=" sc.status!='B' and ";
        }
    ...
    public function GetData_report() {
        ...
        if(isset($requestData['select_inq']) && count($requestData['select_inq']) > 0)
        {
        $status=" scd.si_product_id IN(".implode(",",$requestData['select_inq']).") AND ";
        }
        else
        {
        $status=" scd.si_product_id=0 AND ";
        }
        ...
        foreach ($query as $dt) {
            ...
            $nestedData[] = $cnt++  . "&nbsp;&nbsp;&nbsp; <input class='select' type='checkbox' id='chk' data-id='" . $dt['registed_mobile'] . "' data-mm='" . $dt['registed_mobile'] . "'  cn='" . $dt['contact_person'] . "' value='".$dt['registed_mobile']."' name='id[]' >";
            $nestedData[] = $dt['contact_person'];
            $nestedData[] = $dt['firm_name'];
            $nestedData[] = $dt['registed_mobile']; 
            $nestedData[] = $dt['register_email'];   
...
// If you're serving to IE over SSL, then the following may be needed
    ...
    public function Excel_product_report() { extract($_REQUEST);
        if(!isset($select_inq) || count($select_inq) < 1)
        {
            echo "<script>window.close();</script>"; die;
        } 
        
        /*echo "<pre>"; 
        print_r(implode(',',$select_inq));
        die; */
       ...
       $sql = "SELECT sc.si_clients_id, sc.contact_person, sc.firm_name, sc.registed_mobile, sc.register_email
                ...
                INNER JOIN si_clients_details scd ON sc.si_clients_id=scd.si_clients_id 
                INNER JOIN si_product_purchase spp ON scd.si_clients_details_id=spp.si_clients_details_id 
                where  scd.si_product_id IN(".implode(',',$select_inq).")  AND scd.status !='B' 
                GROUP BY sc.si_clients_id ";
...
// Set document properties
            ...
            //echo"<pre>";  print_r($hr['admin']); 
                ...
                $spreadsheet->setActiveSheetIndex(0)->setCellValue("A$x", $i)
                        ->setCellValue("B$x",$q['contact_person'])
                        ->setCellValue("C$x", $q['firm_name'])
                        ->setCellValue("D$x",$q['registed_mobile'])
                        ->setCellValue("E$x",$q['register_email']);
            ...
            // Set active sheet index to the first sheet, so Excel opens this as the first sheet
            $spreadsheet->setActiveSheetIndex(0);
            $anme='Excel_ProDuctReport'.date('d-m-Y');
            // Redirect output to a clientâ€™s web browser (Excel2007)
            header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
            header('Content-Disposition: attachment;filename="'.$anme.'.xls"');
            header('Cache-Control: max-age=0');
         ...
         redirect(base_url('Client-Report-Product')); 
    }
    
    /*public function dk() // function starts
    { 
    extract($_REQUEST);

    // File and new size
    //$filename = base_url().'assetss/resize/rg.jpg';

    $dir = FCPATH.'assetss/resize/';
    $percent = 0.5;
    define('DG', FCPATH.'assetss/out/');
   ...
   // echo "<pre>"; print_r($newheight); die;

    // Load
    $thumb = imagecreatetruecolor($newwidth, $newheight);
    $source = imagecreatefromjpeg($filename);

    $fn = str_replace("http://localhost/saiinfotech/saicrm/assetss/resize/","",$filename);
