    ...
    public function addData() {
       ...
       if (empty($si_clients_details_id) || $si_clients_details_id[0] == 0) {
        ...
        if (isset($si_clients_id) && $si_clients_id != "") {
            $con = array($this->tbl . '_id' => $si_clients_id);
        ...
        for ($i = 0; $i < count($si_clients_details_id); $i++) {
            ...
            if ($category_id[$i] == 1):
                $p_items = array(
                    ...
                    'si_product_id' => $si_clients_details_id[$i],
...
//            echo $file;
    ...
    public function get_product_data(){
        ...
        $sql = "SELECT cd.*,yearname,DATE_FORMAT(purchase_date, '%d-%m-%Y') as purchase_date,DATE_FORMAT(renewal_date, '%d-%m-%Y') as renewal_date FROM si_clients_details as cd inner join si_for_year as fy on fy.si_for_year_id=cd.si_for_year_id  inner join si_product_purchase  as pd on pd.si_clients_details_id=cd.si_clients_details_id  WHERE cd.si_clients_details_id =".$p_id." AND cd.si_clients_id = ".$id;
         $query = $this->Data_model->Custome_query($sql);
        $json_data = $query;
