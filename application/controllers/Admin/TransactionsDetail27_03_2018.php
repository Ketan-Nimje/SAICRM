...
inner join si_product as p on p.si_product_id=cp.si_product_id where td.status!='B'";
        ...
        foreach ($query as $dt) {
            ...
            $nestedData[] = $dt['transaction_date'];
...
//            echo $file;
			...
			$sql_year="SELECT td.old_lan,cd.si_clients_details_id FROM `si_clients_details` as cd INNER JOIN si_transactions_detail td ON cd.si_clients_details_id=td.si_clients_details_id WHERE cd.si_clients_details_id =".$dt['si_clients_details_id']."  and cd.`serial_no` LIKE '".$dt['serialno']."'";  	
			$p_year=[];
			$p_year = $this->Data_model->Custome_query($sql_year);
			if(isset($p_year[0]['old_lan']))
				$dt['lantotal']=$p_year[0]['old_lan'];
            $nestedData[] = $cnt++;
    ...
    public function get_product_all_purchese() {
        ...
        $sql = "select cd.category_id,cd.new_lan,cd.old_lan,cd.transaction_date,cd.si_transactions_detail_id,cd.status,cd.si_clients_details_id as si_clients_details_id,cd.for_year as foryear, cd.p_name as productname,cd.created_at as renewdate,cd.amount,cd.payment_type,cd.billremarks
                 from si_transactions_detail as cd 
                where cd.si_clients_id='" . $s_id . "' and cd.si_clients_details_id='" . $id . "' and cd.status='A'";
    ...
    public function get_client_edit_data() {
        ...
        if($convertionquery[0]['p_id']!= 0)
            ...
            $c_pid="inner join si_product as p on p.si_product_id=cp.si_product_id";
        $data['product'] = $this->Data_model->Custome_query("SELECT cp.si_clients_details_id,p.p_name FROM `si_clients_details` as cp ".$c_pid." WHERE cp.`si_clients_id` =".$id." and cp.`status`='A' and cp.`status`!='P'");
