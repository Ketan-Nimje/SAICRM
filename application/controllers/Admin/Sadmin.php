...
// datatable column index  => database column name
    ...
    public function update() {
     ...
     function reset_pass() { extract($_REQUEST);
        $sql="select password from si_admin where si_admin_id='$id' ";
        $query=$this->Data_model->Custome_query($sql);
        $ps=$query[0]['password'];
        if($ps!=$curP) { echo json_encode(0); }
   else if($ps==$curP) {  $data = array('password' => $newP);$con = array('si_admin_id' => $id);
