...
class TransactionsDetail extends CI_Controller {
    ...
    public function add_payment_view()
    {
        ...
        $data['product'] =$this->Data_model->Custome_query("SELECT cp.si_clients_details_id,p.p_name, p.si_product_id as product_id, p.sale_amount, p.purchase_amount FROM `si_clients_details` as cp ".$c_pid." WHERE cp.si_clients_id=".$id." and cp.status!='B' and cp.status!='P'");
