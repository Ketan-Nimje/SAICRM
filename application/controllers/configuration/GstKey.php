...
class GstKey extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $this->module_table_prefix = 'si_gstkey_';
        $this->module_table = 'si_gstkey';
        $this->view_data['_controller_path'] = base_url() . $this->module_folder . '/' . $this->module . '/';
        $this->load->model($this->module_folder . '/GstKeyModel', 'ex');
    }
    ...
    function get_data()
    {
        ...
        $sql = "select si_gstkey_id,gstkey,status,created_at,updated_at from si_gstkey where status in ('A', 'D', 'U')";
        ...
        if (!empty(trim($requestData['search']['value']))) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter
            ...
            $sql .= " and ( gstkey  LIKE " . $searchString . ")";
        }
        $query = $this->ex->query($sql);
        ...
        foreach ($query as $row) {
            ...
            $action = "<a data-modal='showModal' data-url='" . $this->view_data['_controller_path'] . "edit/" . $row["si_gstkey_id"] . "' class='edit-row me-1'><i class='btn btn-sm btn-rounded btn-outline-info ri-edit-line'></i></a>
            <a data-url='" . $this->view_data['_controller_path'] . "delete' data-id='" . $row["si_gstkey_id"] . "' data-module='" . $this->module . "' class='delete-row'><i class='btn btn-outline-danger btn-sm btn-rounded ri-delete-bin-line'></i></a>";

            $nestedData = array();
            $nestedData[] = $cnts++;
            $nestedData[] = $row["gstkey"];
            $nestedData[] = display_datetime($row["created_at"]);
            $nestedData[] = display_datetime($row["updated_at"]);
            $nestedData[] = $action;
            $nestedData['DT_RowId'] = "r" . $row['si_gstkey_id'];
            $data[] = $nestedData;
        }
        $json_data = array(
    ...
    function add_update()
    {
        ...
        if ($action == 'added') {
            $is_unique =  '|is_unique[si_gstkey.gstkey]';
        } else {
            $is_unique =  '';
        }

        $this->form_validation->set_rules('gstkey', 'GST Key', 'trim|callback_custom_numeric_dash|required' . $is_unique);
        $this->form_validation->set_message('custom_numeric_dash', 'Invalid GST Key format.');
    ...
    /**
     * Custom validation for gst key
     */
    public function custom_numeric_dash($str)
    {
    ...
    function edit($id = 0)
    {
        ...
        if ($id > 0) {
            ...
            gstkey
            from si_gstkey where status IN ("A", "D", "U") and ' . $this->module_table_prefix . 'id=' . $id;
