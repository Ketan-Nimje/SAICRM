...
class State extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $this->module_table_prefix = 'si_state_';
        $this->module_table = 'si_state';
        $this->view_data['_controller_path'] = base_url() . $this->module_folder . '/' . $this->module . '/';
        $this->load->model($this->module_folder . '/StateModel', 'ex');
    }
    ...
    function get_data()
    {
        ...
        $sql = "select si_state_id,name,status,created_at,updated_at from si_state where status in ('A', 'D')";
        ...
        foreach ($query as $row) {
            ...
            $action = "<a data-modal='showModal' data-url='" . $this->view_data['_controller_path'] . "edit/" . $row["si_state_id"] . "' class='edit-row me-1'><i class='btn btn-sm btn-rounded btn-outline-info ri-edit-line'></i></a>
            <a data-url='" . $this->view_data['_controller_path'] . "delete' data-id='" . $row["si_state_id"] . "' data-module='" . $this->module . "' class='delete-row'><i class='btn btn-outline-danger btn-sm btn-rounded ri-delete-bin-line'></i></a>";
            ...
            $nestedData[] = $action;
            $nestedData['DT_RowId'] = "r" . $row['si_state_id'];
            $data[] = $nestedData;
        }
        $json_data = array(
    ...
    function edit($id = 0)
    {
        ...
        if ($id > 0) {
            ...
            name
            from si_state where status IN ("A", "D") and ' . $this->module_table_prefix . 'id=' . $id;
