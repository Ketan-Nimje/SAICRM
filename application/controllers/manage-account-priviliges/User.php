...
class User extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $this->module_table_prefix = 'si_admin_';
        $this->module_table = 'si_admin';
        ...
        $this->load->model($this->module_folder . '/UserModel', 'ex');
    }
    ...
    function get_data()
    {
        ...
        $sql = "select si_admin_id,username,name,phone,role,otp,create_date,update_date,status from si_admin where status IN ('A', 'D')";
        ...
        foreach ($query as $row) {
            ...
            if ($row['role'] == 'A' || $row['role'] == 'TL') {
                $action .= "<a class='change-status m-1' data-url='" . $this->view_data['_controller_path'] . "change_status' data-id='" . $row["si_admin_id"] . "' data-module='" . $this->module . "' data-status='" . $stts . "'><i class='btn btn-sm btn-rounded " . $icon . "' aria-hidden='true'></i></a>";
            }

            $action .= "<a data-modal='showModal' data-url='" . $this->view_data['_controller_path'] . "edit/" . $row["si_admin_id"] . "' class='edit-row me-1'><i class='btn btn-sm btn-rounded btn-outline-info ri-edit-line'></i></a>";

            if ($row['role'] == 'A' || $row['role'] == 'TL') {
                $action .= "<a data-url='" . $this->view_data['_controller_path'] . "delete' data-id='" . $row["si_admin_id"] . "' data-module='" . $this->module . "' class='delete-row'><i class='btn btn-outline-danger btn-sm btn-rounded ri-delete-bin-line'></i></a>";
            }
    ...
    function add_update()
    {
        ...
        if ($action == 'added') {
            $is_unique =  '|is_unique[si_admin.name]';
        } else {
            $is_unique =  '';
        }
    ...
    function edit($id = 0)
    {
        ...
        if ($id > 0) {
            $sql = "select si_admin_id as id,username,name,phone,password,role from si_admin where status IN ('A', 'D') AND si_admin_id=$id";
    ...
    function delete()
    {
        ...
        if ($this->input->post('id')) {
            $res = $this->ex->update($this->module_table, ['si_admin_id' => $this->input->post('id')], ['status' => 'Y']);
            if ($res) {
    ...
    function change_status()
    {
        ...
        if ($this->input->post('id')) {
            $res = $this->ex->update($this->module_table, ['si_admin_id' => $this->input->post('id')], ['status' => $this->input->post('status')]);
            if ($res) {
                $response['status'] = true;
