...
<body class="fix-header">
                                ...
                                <div class="white-box">
                                        ...
                                        <div class="tab-content">
                                            <div id="home" class="tab-pane fade in active">    
                                                    <div class="panel">
                                                        <div class="panel-heading">Manage State</div>
                                                        <div class="table-responsive">
                                                            <table id="state" class="table table-hover manage-u-table">
                                                                <thead>
                                                                    <tr>
                                            ...
                                            <div id="menu1" class="tab-pane fade">  
                                                    <div class="panel">
                                                      <div id="paneltitle" class="panel-heading">Add State</div> 
                                                      <div id="panelupdate" class="panel-heading">Update State</div> 
                                                    <form name="ff" class="form-horizontal" method="post"
                                                          action="<?php echo base_url() . 'Admin/' . $this->uri->segment(2) . '/addData'; ?>">       
                                                            <div class="form-group">
                                                                <label class="col-md-12">State Name <span class="help"> E.g. "Gujrat"</span></label>
                                                                <div class="col-md-12">
    ...
    <script>
            ...
            var tabs = $('#state').DataTable({
                "processing": true,
                "serverSide": true,
                "ajax": "<?php echo base_url() . 'Admin/State/GetData'; ?>",
                "aoColumnDefs": [{ 'bSortable': false,'aTargets': [0,-1]  }],
            });
            $('#state>tbody').on('click', '.status', function () {
                var id = $(this).data('id');
                var st = $(this).data('status');

                if (st != "A") {
                    var msg = "You won't be deactive this State?";
                    var btn = 'Yes, De-active it!';
                    var title = "De-Activate";
                } else {
                    var msg = "You won't be active this State?";
                ...
                swal({
                    ...
                    if (dismiss === 'cancel') {
                        swal(
                                'Cancelled',
                                'Your imaginary State is safe :)',
                                'error'
                                )
                    }
                })
            });
            $('#state>tbody').on('click', '.delete', function () {
                var id = $(this).data('id');
                var st = $(this).data('status');
                swal({
                    title: 'Are you sure?',
                    text: "You won't be delete this State",
                    type: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data == 1) {
                                swal(
                                        'Successfully!',
                                        'Selected State has been  Deleted.',
                                        'success'
                                        )
                                tabs.draw();
                            }
            ...
            $('#state>tbody').on('click', '.edit', function () {
                var id = $(this).data('id');
                $.ajax({
                    type: 'post',
                    data: {'id': id, 'tbl': tbl},
